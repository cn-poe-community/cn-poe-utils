...
function toPobRarity(frameType: number) {
    return POB_DATA.rarityMap[frameType];
}
...
export class Item {
    ...
    viewModel() {
        ...
        if (model.name === "") {
            ...
            for (const baseType in BASE_TYPE_MAP) {
                if (typeLine.includes(baseType)) {
                    model.typeLine = typeLine.replace(
                        baseType,
                        BASE_TYPE_MAP[baseType],
                    );
                    break;
                }
            }
        ...
        model.quality =
            qualityText !== undefined ? parseInt(qualityText) : undefined;
        ...
        model.enchantMods = (json.enchantMods as string[])
            ?.map((mod) => mod.split("\n"))
            .flat();
        model.implicitMods = (json.implicitMods as string[])
            ?.map((mod) => mod.split("\n"))
            .flat();
        model.explicitMods = (json.explicitMods as string[])
            ?.map((mod) => mod.split("\n"))
            .flat();
        model.craftedMods = (json.craftedMods as string[])
            ?.map((mod) => mod.split("\n"))
            .flat();
        model.fracturedMods = (json.fracturedMods as string[])
            ?.map((mod) => mod.split("\n"))
            .flat();
        model.crucibleMods = (json.crucibleMods as string[])
            ?.map((mod) => mod.split("\n"))
            .flat();
        model.mutatedMods = (json.mutatedMods as string[])
            ?.map((mod) => mod.split("\n"))
            .flat();
        ...
        if (model.baseType === POB_BASE_TYPE_ENERGY_BLADE) {
            ...
            if (abyssalSocketCount > 0) {
                model.explicitMods = [
                    `Has ${abyssalSocketCount} Abyssal Sockets`,
                ];
            }
        }
